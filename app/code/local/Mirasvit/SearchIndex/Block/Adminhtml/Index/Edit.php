...
class Mirasvit_SearchIndex_Block_Adminhtml_Index_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        $this->_addButton('saveandcontinue', array(
            'label' => Mage::helper('searchindex')->__('Save And Continue Edit'),
    ...
    {
        if (Mage::registry('current_model')->getId() > 0) {
            return Mage::helper('searchindex')->__("Edit Search Index '%s'", $this->htmlEscape(Mage::registry('current_model')->getTitle()));
        } else {
            return Mage::helper('searchindex')->__('Add Search Index');
        }
    }

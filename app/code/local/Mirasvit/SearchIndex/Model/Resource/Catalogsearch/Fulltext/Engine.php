...
class Mirasvit_SearchIndex_Model_Resource_Catalogsearch_Fulltext_Engine extends Mage_CatalogSearch_Model_Mysql4_Fulltext_Engine
{
    ...
    public function saveEntityIndexes($storeId, $entityIndexes, $entity = 'product')
    {
        ...
        $this->_addCategoryData($entityIndexes, $storeId)
            ...
            ->_addProductAttributes($entityIndexes, $storeId);

        foreach ($entityIndexes as $entityId => $index) {
            $index['data_index'] = '';
            foreach ($index as $attr => $value) {
                if (!in_array($attr, array('store_id', 'updated', 'fulltext_id'))) {
                    $index[$attr] = Mage::helper('searchindex')->prepareString($value);
                    $index['data_index'] .= ' '.$index[$attr];
                }
            }
            $index['product_id'] = $entityId;
            $index['store_id'] = $storeId;
    ...
    public function prepareEntityIndex($index, $separator = ' ')
    {
        ...
        $productAttributes = $this->getIndexModel()->getIndexInstance()->getAttributes();
        $inlcudeBundled = $this->getIndexModel()->getIndexInstance()->getProperty('include_bundled');

        foreach ($index as $attrKey => $value) {
            if (!$inlcudeBundled && !is_string($index[$attrKey])) {
                $index[$attrKey] = array_shift($index[$attrKey]);
            }

            if (!in_array($attrKey, array_keys($productAttributes))) {
                unset($index[$attrKey]);
            }
        }
    ...
    protected function _addCategoryDataByAttribute(&$index, $storeId, $attrCode)
    {
        ...
        $nameSelect = $adapter->select()
            ->from(
                array('cc' => $this->getTable('catalog/category')),
    ...
    protected function _addTagData(&$index)
    {
        ...
        if (in_array('tags', $tableColumns)) {
            ...
            foreach ($adapter->fetchAll($select) as $row) {
                $index[$row['product_id']]['tags'] .= ' | '.$row['tags'];
            }
        }
    ...
    protected function _addWeights(&$index, $storeId)
    {
        ...
        if ($weightAttr) {
            $columns = array(
                'product_id' => 'product_id',
                // use store scope value, if store scope value IS NULL, then use value of the global scope (0 store)
                'weight' => new Zend_Db_Expr(
                    'SUM(CASE WHEN vc_store.value IS NULL THEN vc_default.value ELSE vc_store.value END)'
                ),
            );

            $select = $adapter->select()
                ->from(array('main_table' => $this->getTable('catalog/category_product_index')), $columns)
                ->joinLeft(
                    array('vc_default' => $weightAttr->getBackend()->getTable()),
                    'category_id = vc_default.entity_id AND vc_default.store_id = 0' .
                        ' AND vc_default.attribute_id = ' . $weightAttr->getId(),
                    array()
                )
                ->joinLeft(
                    array('vc_store' => $weightAttr->getBackend()->getTable()),
                    'category_id = vc_store.entity_id AND vc_store.store_id = main_table.store_id' .
                        ' AND vc_store.attribute_id = ' . $weightAttr->getId(),
                    array()
                )
                ->where('product_id IN (?)', $productIds)
                ->where('main_table.store_id = ?', $storeId)
                ->where('vc_default.value IS NOT NULL OR vc_store.value IS NOT NULL')
                ->group('product_id');

            foreach ($adapter->fetchAll($select) as $row) {
                $index[$row['product_id']]['searchindex_weight'] = intval($index[$row['product_id']]['searchindex_weight']) + intval($row['weight']);
            }
        }
        ...
        if ($weightAttr) {
            ...
            $select = $adapter->select()
                ...
                ->order('store_id ASC') // Sort by store ID, 0 - first
            ;


            // Use "fetchAssoc" to overwrite the values of 0 store with the specific store if exists
            $result = $adapter->fetchAssoc($select);
            foreach ($result as $row) {
                $index[$row['entity_id']]['searchindex_weight'] = intval($index[$row['entity_id']]['searchindex_weight']) + intval($row['weight']);
            }
        }
    ...
    protected function _addEntityIdData(&$index)
    {
        ...
        foreach ($index as $entityId => $value) {
            $index[$entityId]['product_id'] = $entityId;
        }
    ...
    protected function _addCustomOptions(&$index)
    {
        ...
        $select = $adapter->select()
            ...
            ->joinLeft(
                ...
                array('sku' => new Zend_Db_Expr("GROUP_CONCAT(DISTINCT COALESCE(otv.`sku`,main_table.sku) SEPARATOR ' ')"))
            )
            ->joinLeft(
        ...
        foreach ($adapter->fetchAll($select) as $row) {
            $index[$row['product_id']]['name'] .= '|'.$row['title'];
            $index[$row['product_id']]['sku'] .= '|'.$row['sku'];
        }
     ...
     * {@inheritdoc}
     */
    protected function _addProductAttributes(&$index, $storeId)
    {
        foreach ($index as $key => $data) {
            foreach ($data as $property => $value) {
                if ($property != 'updated' && $property != 'searchindex_weight' && empty($value)) {
                    $index[$key][$property] = Mage::getResourceModel('catalog/product')
                        ->getAttributeRawValue($key, $property, $storeId);
                }
            }
        }
    }

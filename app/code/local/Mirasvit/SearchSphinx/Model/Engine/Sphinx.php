...
class Mirasvit_SearchSphinx_Model_Engine_Sphinx extends Mirasvit_SearchIndex_Model_Engine
{
    ...
    protected function _query($query, $storeId, $index, $offset = 1)
    {
        ...
        foreach ($attributes as $attrKey => $weight) {
            $attributes[$attrKey] = intval($weight);
        }
        ...
        if (empty($client->_weights)) {
            $client->SetWeights($attributes);
        }
        ...
        /*
        * Changed $index->getSearchableAttributes() to array_keys($attributes)
        * to search in search attributes , not data_index only
        */
        $sphinxQuery = '@('.implode(',', array_keys($attributes)).')'.$sphinxQuery;
    ...
    protected function _buildQuery($query, $storeId)
    {
        ...
        if (count($result)) {
            $query = $this->prepareFullQuery($query, $result);
        }
    ...
    /**
     * Prepare query before sphinx.
     * Add full escaped search term when customer use several words 
     *
     * @param string $query search term
     * @param array  $result prepared search term after split
     *
     * @return string
     */
    protected function prepareFullQuery($query, $result)
    {
        $baseQuery = '';
        $wordsLimit = Mage::helper('catalogsearch')->getMaxQueryWords();
        if (count(Mage::helper('core/string')->splitWords($query, true, $wordsLimit)) > 1) {
            $baseQuery = '("*'. $this->escapeSphinxQL($query) .'*")';
            if (Mage::getSingleton('searchsphinx/config')->getWildcardMode() !==
                Mirasvit_SearchSphinx_Model_Config::WILDCARD_DISABLED) {
                $baseQuery .= ' | ("'.trim($this->escapeSphinxQL($query)).'")';
            }
             $baseQuery .= ' | ';
        }

        return '('. $baseQuery . implode(' & ', $result).')';
    }
    ...
    {
        return Mage::getModuleDir('etc',
            'Mirasvit_SearchSphinx').DS.'conf'.DS.$this->getSphinxVersion().DS.'sphinx.conf';
    }
    ...
    {
        return Mage::getModuleDir('etc',
            'Mirasvit_SearchSphinx').DS.'conf'.DS.$this->getSphinxVersion().DS.'section.conf';
    }
    ...
    {
        return Mage::getSingleton('searchsphinx/engine_sphinx_native')->getSphinxVersion();
    }
}

...
class Mirasvit_SearchSphinx_Model_Observer
{
    ...
    public function singleResultRedirect($observer)
    {
        if (Mage::getSingleton('searchsphinx/config')->isSingleResultRedictEnabled()) {
            ...
            $result = $collection->getResource()->getReadConnection()->fetchOne($collection->getSelectCountSql());
            if ($result == 1) {
    ...
    public function onProfile(Varien_Event_Observer $observer)
    {
       if (Mage::getSingleton('searchsphinx/config')->isProfilerActive()) {
